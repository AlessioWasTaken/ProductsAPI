...
export const createOrder = (id: string, user: number, product: number, qta: number): Promise<ResultSetHeader> => {
    return new Promise((resolve, reject) => {
        database.query<ResultSetHeader>(`insert into orders(id, user, product, qta) values('${id}', '${user}', '${product}', '${qta}')`, (err, result) => {
            if (err) reject(err);
            resolve(result); 
        });
    });
